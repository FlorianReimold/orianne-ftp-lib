...
orianne::FtpResult orianne::FtpSession::create_new_directory(const std::string& new_directory) {
    if (new_directory[0] != '/') {
        ...
        if (mkdir(path.string().c_str()/*, ACCESSPERMS*/) == 0) {
            return orianne::FtpResult(257, "\"" + path.string() + "\"");
        } else {
...
struct dir_list_dumper : dumper<dir_list_dumper> {
    ...
    explicit dir_list_dumper(boost::function<void (const orianne::FtpResult&)> cb, boost::asio::io_service& service)
    : dumper(cb, service)
    {
    }
...
struct file_dumper : dumper<file_dumper> {
    ...
    explicit file_dumper(boost::function<void (const orianne::FtpResult&)> cb, boost::asio::io_service& service, const std::string& path)
    : dumper(cb, service), stream(path.c_str(), std::ios::in | std::ios::binary), m_buffer(buffer, 1024)
    {
    }
    
    static boost::shared_ptr<file_dumper> create(boost::function<void (const orianne::FtpResult&)> cb, boost::asio::io_service& service, const std::string& path) {
...
struct file_loader : dumper<file_loader> {
    ...
    explicit file_loader(boost::function<void (const orianne::FtpResult&)> cb, boost::asio::io_service& service, const std::string& path)
    : dumper(cb, service), stream(path.c_str(), std::ios::out | std::ios::binary)
    {
    }
    
    static boost::shared_ptr<file_loader> create(boost::function<void (const orianne::FtpResult&)> cb, boost::asio::io_service& service, const std::string& path) {
